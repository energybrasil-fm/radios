...
// ==========================================
// SISTEMA DE QUALIDADE (CORREÇÃO DO MODO AUTOMÁTICO & PERSISTÊNCIA)
// ==========================================
...
function evaluateAutoQuality() {
    // Só avalia se o usuário realmente escolheu ou tem ativo o modo 'auto'
    if (currentQualityMode !== 'auto') return;
    const station = stations[currentStationIndex];
    
    const validQualities = ['high', 'mid', 'low'].filter(k => station.streams[k] && station.streams[k].url && station.streams[k].url.trim() !== "");
    if (validQualities.length <= 1) {
        if (validQualities.length === 1) {
            const onlyQuality = validQualities[0];
...
window.selectQualityMode = function(mode) {
    ...
    if (mode === 'auto') { 
        localStorage.setItem('radioQualityMode', 'auto'); 
        evaluateAutoQuality(); 
    } else { 
...
function updateQualityBadges() {
    const station = stations[currentStationIndex]; 
    const currentStream = station.streams[activeQualityLevel]; 
    const hasValidStream = currentStream && currentStream.url && currentStream.url.trim() !== "";
    ...
    [document.getElementById('mainSpeedBadge'), document.getElementById('stickySpeedBadge')].forEach(b => {
        if (b) { 
            ...
            if (hasValidStream) { 
                // Se o modo global for 'auto' e houver múltiplas qualidades, exibe o ponto pulsante branco 'auto'
                if (currentQualityMode === 'auto' && hasMultipleStreams) {
                    b.classList.add('dot-auto');
                } else {
                    b.classList.add(`dot-${dotCssMap[activeQualityLevel] || 'normal'}`); 
                }
...
function renderQualityMenu() {
    ...
    validQualities.forEach(q => {
        ...
        const isManualSelected = (currentQualityMode === q.id) || (!hasMultipleStreams);
...
window.changeStation = function(index) {
    ...
    // Recupera a preferência original do usuário
    const userPreferredQuality = localStorage.getItem('radioQualityMode') || 'auto';
    currentQualityMode = userPreferredQuality;

    if (currentQualityMode === 'auto') { 
        evaluateAutoQuality(); 
    } else {
        if (!station.streams[currentQualityMode] || !station.streams[currentQualityMode].url) {
            activeQualityLevel = Object.keys(station.streams).find(k => station.streams[k] && station.streams[k].url);
        } else {
            activeQualityLevel = currentQualityMode;
        }
        updateQualityBadges();
    }
