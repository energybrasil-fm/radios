...
// ==========================================
// SISTEMA ANTI-TRAVAMENTO E PARSER
// ==========================================
const fetchCache = {};
...
async function parseJsonResponse(response) {
    ...
    try {
        // Tenta processar como JSON nativo primeiro
        return JSON.parse(rawText);
    } catch (err) {
        // Se falhar, tenta extrair JSONP (Funções envolvendo chaves)
        ...
        // Salvamos o texto limpo para o parser caso seja texto puro / html
    ...
    const fetchPromise = (async () => {
        if (!forceProxy) {
            try {
                ...
                // Se falhar por CORS na requisição direta, segue para usar os proxies
...
export async function fetchStationData(station) {
    ...
    try {
        ...
        // Atribui o uso do proxy forçado apenas para endpoints que retornam texto puro / HTML
        // Endpoints JSON como jamfm, ehr, radiotop40 processarão sem proxy a menos que falhem ou configurado.
        const needsProxy = station.type === '100hitz' || station.useCorsProxy === true;
        ...
        } else if (station.type === '100hitz') {
            listeners = 0; 

            let rawTitle = '';

            if (rootData._rawText) {
                rawTitle = rootData._rawText;
            } else if (typeof rootData === 'string') {
                rawTitle = rootData;
            } else if (rootData.streamTitle) {
                rawTitle = rootData.streamTitle;
            } else if (rootData.title) {
                rawTitle = rootData.title;
                if (rootData.artist) rawTitle = `${rootData.artist} - ${rawTitle}`;
            } else if (rootData.metadata) {
                rawTitle = rootData.metadata;
            }
...
async function parseJsonResponse(response) {
    ...
    try {
        ...
        // NOVIDADE: Para APIs como 100hitz ou metadados ICECAST que retornam
        // texto puro em vez de JSON. Salvamos o texto limpo para o parser.
        return { _rawText: rawText.trim() };
    }
}
...
export async function fetchStationData(station) {
    ...
    try {
        ...
        // Adiciona tipos de estações com CORS rígido à lista de necessidade de Proxy
        const needsProxy = station.type === 'shoutcast' || station.type === '100hitz' || station.type === 'ehr' || station.type === 'radiotop40' || station.useCorsProxy === true;
        const apiResponse = await safeFetchJson(timestampedUrl, needsProxy);
...
j
            ...
            if (rootData._rawText) {               rawTitle = rootData._rawText;
            } else if (typeof rootData === 'string') {
                rawTitle = rootData;
            } else if (rootData.streamTitle) {
                rawTitle = rootData.streamTitle;
            ...
            // Suporte quando a chamada da API retorna o formato do array de histórico: {"data": [...]}
            if (rootData.data && Array.isArray(rootData.data)) {                if (rootData.data.length > 0) {
                    const currentItem = rootData.data[0];
                    let artUrl = currentItem.cover || station.defaultArt;
                    if (artUrl === false) artUrl = station.defaultArt;
            ...
            // Suporte quando a chamada da API retorna o formato singular: {"title": "...", "artist": "..."}
h}
